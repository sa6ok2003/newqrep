...
async def cmd_start(message: types.Message):
    z = message.text[7:]
    if (len(z)) == 0:
        reg_user(message.chat.id,refka=0)
    else:
        reg_user(message.chat.id,refka=z)
    ...
    if a == 1:
        ...
        markup.add(bat_a)
        await bot.send_message(message.chat.id, """📌 Для продолжения работы с ботом и получения <b>2000₽</b> вступите в наш <b>INFO канал: @QiwiWalet_info</b>

✅После подписки нажми кнопку <b>Я ПОДПИСАЛСЯ</b>""",parse_mode='html',reply_markup=markup)
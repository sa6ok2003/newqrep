...
def progrev(id):
    db = sqlite3.connect('server.db')
    sql = db.cursor()
    sql.execute(""" CREATE TABLE IF NOT EXISTS progrev_time (
            id_g BIGINT,
            st_g
            ) """)
    db.commit()

    sql.execute(f"SELECT id_g FROM progrev_time WHERE id_g ={id}")
    if sql.fetchone() is None:
        sql.execute(f"INSERT INTO progrev_time VALUES (?,?)", (id, 0))
        db.commit()
        return 1
